...
Before proceeding:
1. Read `${CLAUDE_PLUGIN_ROOT}/docs/reference/professional-standards.md` — apply the Universal Standards and DISCUSS Phase Standards throughout this phase.
...
```bash
WF="${CLAUDE_PLUGIN_ROOT}/scripts/workflow-cmd.sh" && "$WF" set_active_skill "writing-plans"
```
...
**Level 3 auto-transition:** If autonomy level is 3, invoke `/implement` now after the plan passes review. Do not wait for the user. Only stop if user input is needed during the converge phase (approach selection).
